function Mrend(options) {
    ...
    function Linha(data) {
        ...
        // Mostra a descrição da linha como valor da célula título do grupo
        this.levadesclinha = data.levadesclinha || false;
        // Lista de propriedades que NÃO são herdadas pelas linhas-filha
    ...
    RenderedLinha.prototype.addLinhaFilha = function () {
        ...
        // renderCelula=true: cria uma célula vazia por coluna com linkid=pai
     ...
     * @returns {Array} parsed blacklist array
     */
    ...
    /**
     * Resolve a linha-filha config from a parent config.
     * Procura template filha via linkstamp, usa a sua própria config.
     * Aplica blacklistheranca APENAS se explicitamente configurada.
     *
     * @param {Object} parentConfig - The parent Linha config
     * @param {Object} [filhaRecord] - Optional record (with codigolinha) to disambiguate
     * @returns {Object} filha config (cloned, com blacklist aplicada se necessário)
     */
            ...
            if (recordData) {
                ...
                distinctRowIds.forEach(function (distinctRow) {
                    ...
                    distinctRowFilhas.forEach(function (filha) {
                        // Aplica resolveFilhaConfig para aplicar blacklistheranca
                        var linhaFilhaConfig = resolveFilhaConfig(linhaToRender, filha);
                        if (!linhaFilhaConfig) linhaFilhaConfig = JSON.parse(JSON.stringify(linhaToRender));
                        addLinha(filha, linhaRecords, linhaFilhaConfig, renderedLinhas, true, true, false);
                    });
